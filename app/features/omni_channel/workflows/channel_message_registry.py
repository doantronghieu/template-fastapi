...
class ChannelMessageHandlerRegistry:
    ...
    def set_default_handler(self, handler: ChannelMessageHandler) -> None:
        ...
        logger.debug(f"Default handler set: {handler.__class__.__name__}")
...
def initialize_channel_message_handlers() -> None:
    ...
    2. Loads extension handlers via auto_import_modules()
    3. Finalizes handler selection
    """
    from app.core.autodiscover import ModuleType, auto_import_modules
    ...
    # Load extension message handlers (opt-in via ENABLED_EXTENSIONS)
    auto_import_modules(ModuleType.EXTENSIONS, "message_handlers")
